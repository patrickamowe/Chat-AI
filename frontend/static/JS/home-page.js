...
async function switchActiveConversation(conversationId) {
    ...
    try {
        ...
        messageList.forEach(msg => {
            ...
            if (msg.content) {
                appendMessageBubble(messageStream, 'user', msg.content);
            }
            // 2. Render AI Response Message Bubble
            if (msg.response) {
                appendMessageBubble(messageStream, 'assistant', msg.response);
            }
        });
...
async function submitUserQuery(chatInput) {
    ...
    try {
        ...
        const reply = response.reply || response.content || response.response || (typeof response === 'string' ? response : "");
        const nextId = response.conversation_id || response.id;
        ...
        if (!currentConversationId && nextId) {
            ...
            if (ulConversations) {
                appendHistoryItemToSidebar(ulConversations, {
                    id: nextId,
                    title: queryText.substring(0, 26) + (queryText.length > 26 ? "..." : "")
                });
            }
        }
    } catch (err) {
